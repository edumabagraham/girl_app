...
import { format, eachDayOfInterval, startOfMonth, endOfMonth, addMonths, subMonths, isSameMonth } from 'date-fns';
...
const periodColors: Record<'period' | 'ovulation' | 'fertile' | 'safe', string> = {
...
const selfCareTips: Record<'period' | 'ovulation' | 'fertile' | 'safe', string[]> = {
    period: [
...
export default function MyBody() {
    ...
    const getStatusText = (day: DayInfo | undefined) => {
        ...
        return `${day.prediction.type.charAt(0).toUpperCase() + day.prediction.type.slice(1)} Day`;
    };

    const getCurrentFunFact = () => funFacts[funFactIndex % funFacts.length];

    const getSelfCareTips = (type: 'period' | 'ovulation' | 'fertile' | 'safe') => {
    ...
    const generateCalendarDays = (month: Date) => {
        ...
        return days.map((date, index) => ({
            date,
            prediction: {
                type: mockPredictions[index % 4] as 'safe' | 'fertile' | 'ovulation' | 'period',
                probability: Math.random(),
            },
            actualPeriod: null,
            isSelected: formatDate(date, 'yyyy-MM-dd') === formatDate(selectedDate, 'yyyy-MM-dd'),
        }));
    };
            ...
            <div className="flex justify-around bg-white shadow rounded-lg p-4">
                {['Period', 'Vagina', 'Breast', 'Hormones'].map((tab) => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg font-medium transition ${activeTab === tab ? 'bg-red-500 text-white' : 'bg-gray-100 text-red-600 hover:bg-red-100'
                            }`}
                    >
                        {tab}
                    </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                    ...
                    >
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <h2 className="text-lg font-semibold text-red-600">{format(calendarMonth, 'MMMM yyyy')}</h2>
                            <button
                        ...
                        <div className="grid grid-cols-7 gap-2">
                            {calendarDays.map((day) => (
                                <motion.button
                                    ...
                                    className={`w-8 h-8 rounded-full text-xs font-medium flex items-center justify-center ${day.prediction.type ? periodColors[day.prediction.type] : 'bg-gray-200'
                    ...
                    >
                        <div className="w-48 h-48 bg-gradient-to-r from-red-300 to-red-600 rounded-full shadow-lg flex flex-col items-center justify-center text-white text-center">
                            <p className="text-2xl font-semibold">{formatDate(selectedDate, 'MMM d, yyyy')}</p>
                            <p className="text-lg">{getStatusText(calendarDays.find((d) => d.isSelected))}</p>
                        </div>
                    </motion.div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <motion.div
                        ...
                        transition={{ duration: 0.5 }}
                    >
                        <h2 className="text-lg font-semibold text-red-600 mb-4">How Are You Feeling Today?</h2>
                        <textarea
                            value={userFeelings}
                    ...
                    <motion.div
                        ...
                        transition={{ duration: 0.5, delay: 0.1 }}
                    >
                        <h2 className="text-lg font-semibold text-red-600 mb-4">Did You Know?</h2>
                        <AnimatePresence>
                            <motion.div
                                key={funFactIndex}
                                initial={{ opacity: 0, y: 20 }}
                    ...
                    <motion.div
                        ...
                        transition={{ duration: 0.5, delay: 0.2 }}
                    >
                        <h2 className="text-lg font-semibold text-red-600 mb-4">Self-Care Tips</h2>
                        <AnimatePresence>
                            <motion.div
                                key={selfCareIndex}
                                initial={{ opacity: 0, y: 20 }}
                            ...
                            >
                                <ul className="space-y-2 text-gray-700">
                                    {getSelfCareTips(calendarDays.find((d) => d.isSelected)?.prediction.type ?? 'safe').map(
                                        (tip, index) => (
                                            <li key={index}>{tip}</li>
                                        )
                                    )}
